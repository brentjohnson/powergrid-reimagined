...
class PowerGridSingleAgentEnv(gym.Env):
    ...
    def __init__(
        ...
        # Snapshot bytes (PGRLPOL6) for "policy" opponents; applied at reset.
    ...
    def set_opponent_policy(self, data: bytes) -> None:
        ...
        the learner's policy in PGRLPOL6 bytes (powergrid_env.export). Applies
    ...
    def set_opponent_pool(self, entries: list[tuple[str, object, float]]) -> None:
        ...
        weight): kind "policy" with PGRLPOL6 bytes, or "bots" with a heuristic
        ...
        for kind, payload, weight in entries:
            if kind == "policy":
                if not isinstance(payload, bytes):
                    raise ValueError("'policy' pool entries need PGRLPOL6 bytes")
            elif kind == "bots":
