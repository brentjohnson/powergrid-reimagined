...
`PGRLPOL3`), so every prior checkpoint is invalid and there is no common
...
| `--out` (../assets/policies/expert.bin) | Destination for the flat `PGRLPOL3` binary weights file |
...
- **GNN map encoder** — consciously skipped: the in-game Expert bot's Rust
  inference (`PGRLPOL3`) only runs a fixed two-hidden-layer MLP, and a policy
