...
- Fast native methods (no JSON, numpy in/out): `observation(actor)`, `action_mask(actor)`, `apply_action_id(actor, id)`, `step_self_play(id)` (fused self-play step; also returns the acting seat's powered-cities delta vs the other seats' mean — zero-sum reward shaping, so a shared policy can't farm the bonus by stalling), `step_vs_bots(learner, id, difficulty)` + `advance_bots(learner, difficulty)` (fused vs-bots step, bots driven inside Rust).
