...
- `encoding.rs` — the RL observation/action encoding (moved here from powergrid-py so the Expert bot can use it): `CITY_IDS`/`REGION_NAMES`/`OBS_SIZE` (507)/`N_ACTIONS` (94) constants, `build_observation`, `build_action_mask`, `action_id_to_action`, `compute_legal_move_info`, `current_actor_id`, `map_matches_default`. Compiled against the **default (USA) map**; mirrors `python/src/powergrid_env/constants.py` (parity tests in `python/tests/test_native_bridge.py` catch drift). **2026-07-08:** obs grew 454→507 with two features the heuristic bot relies on but the net previously couldn't see — per-city connection cost from the actor's network (`Map::connection_costs_from`, the Dijkstra routing that drives build decisions) and opponent per-resource fuel demand (market contention). Any change to the obs layout invalidates all trained checkpoints. Auction bidding (`PLACE_BID_BASE`, `N_BID_ACTIONS = 1`) is English-auction style — the only representable raise is +1 over the standing bid; `PassAuction` covers dropping out. (Previously 50 actions encoded raise offsets +1..+50; collapsed to 1 because self-play with that range learned to jump-bid by large, non-strategic amounts instead of the human "+1 at a time" convention — see `project_training_reset` memory.)
- `policy.rs` — native inference for the Expert policy: `MlpPolicy` (507 → H → tanh → H → tanh → 94 logits, where the input width matches `OBS_SIZE` and the hidden width H is read from the policy file header — the trainer's `--net-width` sets it for fresh runs (default 128; the embedded `expert.bin` is still 64-wide); the architecture is fixed at two equal-width hidden layers — plain-loop forward pass, no ML deps), `sample_masked`, `default_policy()` (parses `assets/policies/expert.bin`, embedded at compile time, once via `OnceLock`; `RL_POLICY_FILE` env var overrides). Weights are produced by `python/scripts/export_policy.py` from an sb3 MaskablePPO checkpoint; a golden-logits test pins Rust output to torch. On a non-default map or missing/corrupt weights, Expert bots degrade to the hard-style heuristic (warn log, game still starts). **2026-06-24:** the embedded `expert.bin`/`expert.golden.json` are stale 143-action exports (pre auction-bid-encoding-collapse, see below) and fail `MlpPolicy::from_bytes`'s dim check — Expert bots currently fall back to the heuristic until a 94-action policy is trained and re-exported; the golden-logits test and the expert-policy integration tests are `#[ignore]`d in the meantime.
...
egui desktop tool for interactively inspecting the RL Expert policy network (`powergrid_bot_strategy::policy::MlpPolicy`, 507 → H → tanh → H → tanh → 94, hidden width H read from the policy file header — 64 by default). Loads a `PGRLPOL1` policy file (CLI arg) or the embedded `expert.bin` (no arg), lets you edit the 507-dim observation with labeled sliders grouped by section, and renders the live forward pass as a four-column node graph (input cells, both hidden layers, output logits). Clicking a node traces its full weighted path through every layer (e.g. an output node back through hidden2 → hidden1 → input, an input cell forward through hidden1 → hidden2 → output) — edges are colored by sign and scaled by magnitude, normalized independently per weight matrix (input→h1 / h1→h2 / h2→out) so one large weight doesn't wash out the rest; hovering shows exact values. The right panel lists all 94 actions sorted by logit with softmax probabilities.

- `main.rs` — self-contained eframe app (`NetViz`). Usage: `cargo run -p powergrid-netviz [-- path/to/policy.bin]`.
- `obs_layout.rs` — labeled sections of the 507-dim observation vector, mirroring `encoding::build_observation`'s numbered layout (money, resources, plants, city one-hots, market, phase scalars, per-city connection cost, opponent fuel demand, etc.).
- `action_labels.rs` — human-readable names for all 94 actions, derived from `encoding`'s action-base-index constants and `CITY_IDS`.
