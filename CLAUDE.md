...
- `policy.rs` — native inference for the Expert policy: `MlpPolicy` (582 → H → tanh → H → tanh → 26 logits, where the input width matches `OBS_SIZE`, the output width `N_ACTIONS`, and the hidden width H is read from the policy file header — the trainer's `--net-width` sets it for fresh runs (default 128; the embedded `expert.bin` is still 64-wide); the architecture is fixed at two equal-width hidden layers — plain-loop forward pass, no ML deps), `sample_masked`, `default_policy()` (parses `assets/policies/expert.bin`, embedded at compile time, once via `OnceLock`; `RL_POLICY_FILE` env var overrides). Weights are produced by `python/scripts/export_policy.py` from an sb3 MaskablePPO checkpoint; a golden-logits test pins Rust output to torch. On a non-default map or missing/corrupt weights, Expert bots degrade to the hard-style heuristic (warn log, game still starts). **2026-06-24, still true after the Phase-2 macro rebuild:** the embedded `expert.bin`/`expert.golden.json` are stale 143-action exports and fail `MlpPolicy::from_bytes`'s dim check — Expert bots fall back to the heuristic until a **26-macro** policy is trained and re-exported; the golden-logits test and the expert-policy integration tests are `#[ignore]`d in the meantime.
