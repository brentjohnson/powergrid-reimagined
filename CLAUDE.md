...
- `macro_actions.rs` — the **macro action space** the RL policy plays (`N_MACROS = 26`). Instead of the ~600 primitive micro-decisions per game that capped every earlier learner (compounding error over per-unit `BuildCity`/`BuyResources` sequences), the policy picks **one complete phase-plan per turn** (~50 decisions/game) which expands to a short primitive sequence the engine already accepts as a whole-turn batch (`BuildCities`, `BuyResourceBatch`). Layout: nominate market slot 0–5 (0–5), auction pass (6), auction raise +1 (7), build nothing/default/cheapest-1/2/3/max/block/race (8–15), buy nothing/default/stockpile-2/stockpile-3/denial (16–20), discard plant slot 0–2 (21–23), power optimal/nothing (24–25). Key invariants: only the *learner* plays macros (heuristic opponents keep `strategy::decide_with_bot`); the `*_DEFAULT` macros delegate to the champion `hard` profile unchanged so "play the heuristic" is always representable **bit-exactly** (Gate 0 test), making the other menu items isolated alternatives that can only make themselves worse; fuel splits and resource discards are auto-resolved heuristically (`resolve_auto_phases`) so they never consume a policy decision. `legal_macros` masks by trial application on a clone **and dedups** — a macro whose expansion equals a lower-id macro's is illegal, so e.g. `CHEAPEST_3` collapses onto `CHEAPEST_2` when only 2 are affordable. `teacher_macro_id` gives the imitation/DAgger label. Alternative *build* plans price cities with the heuristic's own cash rule (full money for cities within powering headroom; only overbuild past headroom is gated behind the fuel/auction reserve) — flat-capping the whole city budget at `money - fuel_reserve` zeroed it in ~85% of real decisions and silently deduped the entire alternative-build menu away.
